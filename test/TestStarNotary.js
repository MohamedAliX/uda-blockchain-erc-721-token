...
it('can add the star name and star symbol properly', async() => {
    ...
    let tkName = await instance.name();
    let tkSymbol = await instance.symbol();
    assert.equal(tkName, 'Charlize Star');
    assert.equal(tkSymbol, 'CZS');
    let starName = await instance.tokenIdToStarInfo(starId);
...
it('lets 2 users exchange stars', async() => {
    ...
    let user1 = accounts[1];
    let user2 = accounts[2];
    let starId1 = starTest.nextId();
    let starId2 = starTest.nextId();
    ...
    // 4. Call the exchangeStars functions implemented in the Smart Contract
    await instance.exchangeStars(starId1, starId2, {from: user1});
    // 5. Verify that the owners are restored
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user = accounts[0];
    let starId = starTest.nextId();
    await instance.createStar('awesome czs', starId, {from: user});
    // 2. Call your method lookUptokenIdToStarInfo
    let starName = await instance.lookUptokenIdToStarInfo(starId);
    // 3. Verify if you Star name is the same
    assert.equal(starName, 'awesome czs');
});